...
export const FilterBar: React.FC<FilterBarProps> = ({
  ...
  return (
    <div className="bg-[#111] border border-white/10 rounded-2xl p-4 sm:p-5 mb-6 shadow-2xl space-y-4">
        ...
        <div className="flex items-center justify-between md:justify-end gap-3 w-full md:w-auto">
          ...
          <div className="flex items-center gap-1.5 text-xs text-white/60">
            {onSelectAllWatched && (
              <button
                ...
                className="hover:text-emerald-400 text-white/80 bg-[#050505] hover:bg-emerald-950/40 border border-white/10 hover:border-emerald-800/50 px-3 py-2 rounded-xl transition-colors text-xs font-bold cursor-pointer flex items-center gap-1"
            ...
            {onUnselectAllWatched && (
              <button
                ...
                className="hover:text-white text-white/40 bg-[#050505] hover:bg-white/10 border border-white/10 px-3 py-2 rounded-xl transition-colors text-xs font-bold cursor-pointer"
                title="Décocher tout"
              >
                Décocher tout
              </button>
          ...
          <div className="bg-[#050505] p-1 rounded-xl border border-white/10 flex items-center gap-1">
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-black uppercase italic transition-all cursor-pointer ${
                viewMode === 'grid'
      ...
      <div className="overflow-x-auto pb-1 scrollbar-thin border-t border-b border-white/10 py-3">
        <div className="flex items-center gap-2 min-w-max">
          <span className="text-[10px] font-black uppercase text-[#E62429] tracking-widest mr-1 flex items-center gap-1">
            <Tv className="w-3.5 h-3.5 text-[#E62429]" /> Platforme Streaming :
          </span>
          ...
          >
            <span className="w-2.5 h-2.5 rounded-full bg-[#006E99]" />
            Disney+
          </button>
          ...
          >
            <span className="w-2.5 h-2.5 rounded-full bg-[#00A8E1]" />
            Prime Video
          </button>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 pt-3 border-t border-white/10 text-xs">
        ...
        <div className="flex items-center gap-2">
          <ArrowUpDown className="w-3.5 h-3.5 text-[#E62429]" />
          <span className="text-white/40 font-bold uppercase text-[10px] tracking-wider">Trier par :</span>
          <select
            value={selectedSort}
            onChange={(e) => onSortChange(e.target.value as SortOption)}
            className="bg-[#050505] text-white text-xs rounded-xl px-3 py-1.5 border border-white/10 focus:outline-none focus:border-[#E62429] cursor-pointer font-bold"
          >
            <option value="chronological">⏱️ Ordre Chronologique (Histoire)</option>
            <option value="release">🎬 Ordre de Sortie (Cinéma)</option>
            <option value="duration_asc">⏳ Durée (Du + court au + long)</option>
            <option value="duration_desc">⏳ Durée (Du + long au + court)</option>
